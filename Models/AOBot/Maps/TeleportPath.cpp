...
int CTeleportPath::TP_RANGE;

CTeleportPath::CTeleportPath(boost::numeric::ublas::matrix<WORD> pCollisionMap, int cx, int cy, int range) {
    CTeleportPath::TP_RANGE = range;
...
BOOL CTeleportPath::MakeDistanceTable() {
    ...
    for (unsigned int x = 0; x < m_ppTable.size1(); x++) {
        for (unsigned int y = 0; y < m_ppTable.size2(); y++) {
...
BOOL CTeleportPath::GetBestMove(POINT &pos, int nAdjust) {
    if (CalculateDistance(m_ptEnd, pos) <= CTeleportPath::TP_RANGE) {
        pos = m_ptEnd;
    ...
    for (p.x = pos.x - TP_RANGE; p.x <= pos.x + TP_RANGE; p.x++) {
        for (p.y = pos.y - TP_RANGE; p.y <= pos.y + TP_RANGE; p.y++) {
            if (!IsValidIndex(p.x, p.y)) //why would you need this for teleporting??
                continue;
...
DWORD CTeleportPath::FindTeleportPath(POINT ptStart, POINT ptEnd, std::vector<POINT> &lpBuffer, DWORD dwMaxCount) {
...
//		return 0;
    ...
    MakeDistanceTable();
    DumpDistanceTable("maps/distance.txt");
    ...
    while (nRes != PATH_FAIL && dwFound < dwMaxCount) {
        ...
        // keep trying...
        while (nRes == PATH_FAIL && dwFound > 0) {
            char temp[50];
            sprintf_s(temp, sizeof(temp), "maps/distance_%d.txt", dwFound);
            DumpDistanceTable(temp);
            dwFound--;
            pos = lpBuffer[dwFound];
            nRes = GetBestMove(pos);
        }
    }

    if (!bOK) {
        lpBuffer.resize(dwFound);
        CCollisionMap::last->DumpMap("fail.txt", lpBuffer);
        dwFound = 0;
    }
...
int CTeleportPath::GetRedundancy(const std::vector<POINT> lpPath, DWORD dwMaxCount, const POINT &pos) {
