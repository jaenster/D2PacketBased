...
static int CompressDataContent(const void* inData, int inLength, void* outData, int outLength)
{
    static const unsigned int CompressionTable[] =
            {
	...
	return static_cast<int>(outdata - tmp);
}

static int DecompressDataContent(const void* inData, int inLength, void* outData, int outLength)
{
    static const unsigned int CharIndex[]  =
            {
    ...
    static const unsigned char CharTable[] =
            {
   ...
   size = inLength;
   inptr = static_cast<const unsigned char*>(inData);

   maxcnt = outLength;
   outptr = static_cast<unsigned char*>(outData);
   cnt = 0x20;
...
static int ComputeDataLength(const void* buffer)
{
   ...
   return ((bytes[0] & 0xf) << 8) + bytes[1];
}

static int ComputeDataHeaderLength(const void* buffer) 
{
	const unsigned char* bytes = static_cast<const unsigned char*>(buffer);
	return bytes[0] < 0xf0 ? 1 : 2;
}

static int ComputeDataContentLength(const void* buffer)
{
	return ComputeDataLength(buffer) - ComputeDataHeaderLength(buffer);
}

static int CreateDataHeader(int contentLength, void* header, int maxHeaderLength)
{
	...
	if (contentLength > 238)
	{
		...
		if (maxHeaderLength < headerLength)
			return -1;
	...
	else
	{
		...
		dataLength = contentLength + headerLength;
		bytes[0] = static_cast<unsigned char>(dataLength);
	}

	return headerLength;
}

int Compression::CompressPacket(const void* inData, int inLength, void* outData, int outLength)
{
	int contentLength = CompressDataContent(inData, inLength, outData, outLength);
	if (contentLength == -1)
	{
		return -1;
	}

	int headerLength = CreateDataHeader(contentLength, outData, outLength);
	if (headerLength == -1)
	{
		return -1;
	}

	int dataLength = CompressDataContent(inData, inLength, static_cast<unsigned char*>(outData) + headerLength, outLength - headerLength);
	if (dataLength != -1)
	{
...
int Compression::GetPacketSize(const void* data, int size)
{
	return ComputeDataLength(data);
}
